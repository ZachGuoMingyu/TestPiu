    ...
    children: [
      {
        path: 'list',
        name: 'custormer',
        component: () => import('@/pages/custormer/index'),
        meta: { title: '顾客管理', icon: 'peo' }
      },
      // 顾客详情路由配置
      {
        path: 'custormerDetails',
        name: 'custormerDetails',
        // 因为我们不希望它出现在左侧菜单栏中 所以我们给其设置一个属性
        hidden: true,
        component: () => import('@/pages/custormer/custormerDetails'),
        meta: { title: '顾客详情', icon: 'peo' }
        // 配置完以后 回到顾客页面 点击详情按钮跳转到顾客详情
      }
    ]
  },
  // 产品管理
